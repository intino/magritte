...
public class Getters_referenceTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "Variable & reference & owner")), not(condition("type", "target")), not(condition("type", "concept")), not(condition("type", "inherited")), not(condition("type", "overriden")), (condition("trigger", "getter"))).add(literal("public ")).add(mark("type", "variableType")).add(literal(" ")).add(mark("name", "javaValidWord", "firstLowerCase")).add(literal("() {\n\treturn ")).add(mark("name", "javaValidWord", "firtLowercase")).add(literal(".instance() == null ? null : ")).add(mark("name", "javaValidWord", "firtLowercase")).add(literal(".instance().as(")).add(mark("type", "reference")).add(literal(".class);\n}"))
		);
		return this;
	}
}
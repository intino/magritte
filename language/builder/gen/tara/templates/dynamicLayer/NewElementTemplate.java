...
public class NewElementTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "Variable & reference & multiple")), (condition("trigger", "assign"))).add(mark("name", "firstLowerCase")).add(literal(" = ")).add(mark("name")).add(literal(".stream().map(r -> new tara.magritte.Reference(r._instance())).collect(java.util.stream.Collectors.toList());")),
			rule().add((condition("type", "Variable & reference")), not(condition("type", "concept")), (condition("trigger", "assign"))).add(mark("name", "firstLowerCase")).add(literal(" = new tara.magritte.Reference(")).add(mark("name")).add(literal("._instance());")),
			rule().add((condition("type", "variable")), (condition("trigger", "assign"))).add(mark("name", "firstLowerCase")).add(literal(" = ")).add(mark("name", "firstLowerCase")).add(literal(";"))
		);
		return this;
	}
}
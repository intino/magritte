...
public class NewElementTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "Variable & reference & multiple")), (condition("trigger", "assign"))).add(literal("newElement._instance().set(newElement, \"")).add(mark("name", "firstLowerCase")).add(literal("\", ")).add(mark("name")).add(literal(".stream().map(r -> new tara.magritte.Reference(r._instance())).collect(java.util.stream.Collectors.toList()));")),
			rule().add((condition("type", "Variable & reference")), not(condition("type", "concept")), (condition("trigger", "assign"))).add(literal("newElement._instance().set(newElement, \"")).add(mark("name", "firstLowerCase")).add(literal("\", java.util.Collections.singletonList(new tara.magritte.Reference(")).add(mark("name")).add(literal("._instance())));")),

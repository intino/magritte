...
public class SettersTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "Variable & file")), not(condition("type", "target")), not(condition("type", "inherited")), not(condition("type", "overriden")), not(condition("type", "multiple")), not(condition("type", "final")), (condition("trigger", "setter"))).add(literal("public void ")).add(mark("name", "firstLowerCase")).add(literal("(String destinationPath, java.net.Url url) {\n\tthis.")).add(mark("name", "firstLowerCase")).add(literal(" = _model().save(destinationPath, url);\n}\n\npublic void ")).add(mark("name", "firstLowerCase")).add(literal("(String destinationPath, java.io.ImputStream stream) {\n\tthis.")).add(mark("name", "firstLowerCase")).add(literal(" = _model().save(destinationPath, stream);\n}")),

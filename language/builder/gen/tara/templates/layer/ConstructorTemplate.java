...
public class ConstructorTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "Variable & reference")), (condition("type", "owner")), not(condition("type", "inherited | empty")), (condition("slot", "values")), (condition("type", "multiple")), (condition("trigger", "constructor"))).add(literal("_load(\"")).add(mark("name", "firstLowerCase")).add(literal("\", new java.util.ArrayList<>(java.util.Arrays.asList(model().conceptOf(\"")).add(mark("type")).add(literal("\"))));")),

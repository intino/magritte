...
public class ConstructorTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "Variable & file")), (condition("type", "owner")), not(condition("type", "inherited")), (condition("Slot", "values")), (condition("type", "multiple")), (condition("trigger", "constructor"))).add(literal("_load(\"")).add(mark("name", "firstLowerCase")).add(literal("\", new java.util.ArrayList<>(java.util.Arrays.asList(_model().conceptOf(\"")).add(mark("type")).add(literal("\"))));")),
			...
			rule().add((condition("type", "Variable")), (condition("type", "owner")), not(condition("type", "inherited")), (condition("Slot", "values")), (condition("trigger", "constructor"))).add(literal("_load(\"")).add(mark("name", "firstLowerCase")).add(literal("\", tara.magritte.loaders.ResourceLoader.load(Arrays.asList(")).add(mark("values", "quoted").multiple(", ")).add(literal("), _model()));"))
		);
		return this;
	}
}
...
public class ViewerTemplate extends Template {

	protected ViewerTemplate(Locale locale, LineSeparator separator) {
		super(locale, separator);
	}

	public static Template create() {
		return new ViewerTemplate(Locale.ENGLISH, LF).define();
	}

	public Template define() {
		add(
			rule().add((condition("type", "model"))).add(literal("package ")).add(mark("name", "LowerCase")).add(literal(";\n\nimport tara.magritte.Model;\nimport tara.magritte.Viewer;\n\nimport java.util.List;\n\npublic class ")).add(mark("name", "javaValidName")).add(literal("Viewer extends Viewer {\n\t")).add(mark("node", "declaration").multiple("\n")).add(literal("\n\n\tpublic ")).add(mark("name", "javaValidName")).add(literal("Viewer(Model model) {\n\t    ")).add(mark("node", "assign").multiple("\n")).add(literal("\n\t}\n\n\t")).add(mark("node", "getter").multiple("\n\n")).add(literal("\n}")),

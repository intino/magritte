...
public class ModelHandlerTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "node")), not(condition("type", "single")), (condition("trigger", "filter"))).add(literal("public List<")).add(mark("qn", "reference")).add(literal("> ")).add(mark("name", "firstLowerCase")).add(literal("List(java.util.function.Predicate predicate) {\n    return ")).add(mark("name", "firstLowerCase")).add(literal("List.stream().filter(predicate).collect(java.util.stream.Collectors.toList());\n}")),
			rule().add((condition("type", "node")), not(condition("type", "final")), (condition("trigger", "new"))).add(literal("public ")).add(mark("qn", "reference")).add(literal(" new")).add(mark("name", "firstUpperCase")).add(literal("(String spacename, String name")).add(expression().add(literal(", ")).add(mark("variables", "list").multiple(", "))).add(literal(") {\n\t")).add(mark("qn", "reference")).add(literal(" newElement = model.newMain(")).add(mark("qn", "reference")).add(literal(".class, spacename, name).as(")).add(mark("qn", "reference")).add(literal(".class);\n\t")).add(expression().add(mark("variable", "assign").multiple("\n"))).add(literal("\n\treturn newElement;\n\n}\npublic ")).add(mark("qn", "reference")).add(literal(" new")).add(mark("name", "firstUpperCase")).add(literal("(String spacename")).add(expression().add(literal(", ")).add(mark("variables", "list").multiple(", "))).add(literal(") {\n    ")).add(mark("qn", "reference")).add(literal(" newElement = model.newMain(")).add(mark("qn", "reference")).add(literal(".class, spacename).as(")).add(mark("qn", "reference")).add(literal(".class);\n    ")).add(expression().add(mark("variable", "assign").multiple("\n"))).add(literal("\n    return newElement;\n}"))
		);
		return this;
	}
}
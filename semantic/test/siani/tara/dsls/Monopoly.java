...
public class Monopoly extends Tara {
	public Monopoly() {
		in("").def(context("").allow(single("Dices"), multiple("RecurrentJob"), multiple("Map"), multiple("StartJob"), multiple("Simulation"), multiple("EndJob"), name()).require(oneOf(_single("LuckyCards"), _single("CommunityCards")), _multiple("Player"), _single("Board"), _single("LuckyCards"), _single("CommunityCards")));
		in("Board").def(context("Entity", "Concept").allow(multiple("Init"), multiple("Terrain"), multiple("Community"), multiple("Taxes"), multiple("Station"), multiple("Luck"), multiple("Jail"), multiple("Company"), multiple("FreeParking"), multiple("GoToJail"), parameter("squareAt", "native", false, 0, "SquareAt#public Square square(int position)"), parameter("squareOf", "native", false, 1, "SquareOf#public Square squareOf(Class<? extends Square> aClass)"), parameter("positionOf", "native", false, 2, "Position#public int position(Square square)"), name()));
		in("Dices").def(context("Entity", "Concept").allow(parameter("value1", "integer", false, 0, ""), parameter("value2", "integer", false, 1, ""), parameter("roll", "native", false, 2, "Roll#public void execute()"), parameter("doubles", "native", false, 3, "Doubles#public boolean check()"), name()));
		...
		in("DoTurn").def(context("Action", "Concept").allow(name()).assume(isFeatureInstance(), isImplicitInstance()));
		in("PlayerIsJailed").def(context("Entity", "Concept").allow(parameter("rule", "native", false, 0, "Rule#public boolean apply()"), name()).assume(isFeatureInstance(), isImplicitInstance()));
		in("JailAfterThreeDoubles").def(context("Entity", "Concept").allow(parameter("rule", "native", false, 0, "Rule#public boolean apply()"), name()).assume(isFeatureInstance(), isImplicitInstance()));
		in("Advance").def(context("Entity", "Concept").allow(parameter("rule", "native", false, 0, "Rule#public boolean apply()"), name()).assume(isFeatureInstance(), isImplicitInstance()));
		in("ToJailWhenInGoToJailSquare").def(context("Entity", "Concept").allow(parameter("rule", "native", false, 0, "Rule#public boolean apply()"), name()).assume(isFeatureInstance(), isImplicitInstance()));
		in("CheckCard").def(context("Entity", "Concept").allow(parameter("rule", "native", false, 0, "Rule#public boolean apply()"), name()).assume(isFeatureInstance(), isImplicitInstance()));
		in("DicesResult").def(context("Entity", "Concept").allow(name()).require(_parameter("value1", "integer", false, 0, ""), _parameter("value2", "integer", false, 1, "")).assume(isFeatureInstance(), isImplicitInstance()));

package io.intino.tara.plugin.codefolding;
...
class StringFoldingBuilder {
	...
	void processMultiValuesParameters(@NotNull List<FoldingDescriptor> descriptors, Node node) {
		descriptors.addAll(searchMultiValuedParameters(node).stream().
			map(multivalued -> new FoldingDescriptor((PsiElement) multivalued, getRange(((io.intino.tara.plugin.lang.psi.Valued) multivalued).getValue())) {
	...
	private void searchMultiLineVarInit(Node node, List<PsiElement> strings) {
		...
		for (Parameter parameter : ((TaraNode) node).getBody().getVarInitList()) {
			Value value = ((io.intino.tara.plugin.lang.psi.Valued) parameter).getValue();
			if (!Primitive.STRING.equals(((io.intino.tara.plugin.lang.psi.Valued) parameter).getInferredType())) continue;
	...
	private boolean hasStringValue(Variable variable) {
		return ((io.intino.tara.plugin.lang.psi.Valued) variable).getValue() != null && !((io.intino.tara.plugin.lang.psi.Valued) variable).getValue().getStringValueList().isEmpty();
	}

...
public class ExpressionInjectionTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
				rule().condition((type("function"))).output(literal("package ")).output(mark("generatedLanguage", "lowerCase")).output(literal(".natives;\n\n")).output(mark("imports").multiple("\n")).output(literal("\n\nclass ")).output(mark("name", "FirstUpperCase")).output(literal(" implements")).output(expression().output(literal(" ")).output(mark("scope", "lowercase")).output(literal(".functions.")).next(expression().output(literal(" ")))).output(mark("rule", "FirstUpperCase")).output(literal(", io.intino.tara.magritte.Function {\n\t")).output(mark("nativeContainer")).output(literal(" self;\n\n\t@Override\n\t")).output(mark("signature")).output(literal(" {\n\t\t")).output(mark("return")),

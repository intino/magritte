...
public class GroovyNativeImplementationToTara extends RelatedItemLineMarkerProvider {
	...
	protected void collectNavigationMarkers(@NotNull PsiElement element, Collection<? super RelatedItemLineMarkerInfo> result) {
		...
		if (destiny == null || !isAvailable(psiClass, outPackage(element))) return;
	...
	private String outPackage(@NotNull PsiElement element) {
		String graphPackage = TaraUtil.graphPackage(element);
		return graphPackage.isEmpty() ?
				ModuleProvider.moduleOf(element).getName() :
				graphPackage;
	}
}
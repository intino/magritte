...
public class TaraMoveNodeDialog extends RefactoringDialog {
	private TaraMoveNodePanel myPanel;

	public TaraMoveNodeDialog(@NotNull Project project, @NotNull PsiNamedElement[] elements, @Nullable String destination) {
		super(project, true);
		...
		if (elements.length == 1) {
			...
			if (e instanceof Node) moveText = MessageProvider.message("refactoring.move.concept", e.getName());
		}
		if (destination == null) destination = getContainingFileName(elements[0]);

		myPanel = new TaraMoveNodePanel(moveText, destination);

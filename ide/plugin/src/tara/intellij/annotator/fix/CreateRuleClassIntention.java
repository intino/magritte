...
public class CreateRuleClassIntention extends ClassCreationIntention {
	...
	public CreateRuleClassIntention(Rule rule) {
		...
		if (facet == null) this.rulesPath = RULES_PACKAGE;
			...
			this.nativesPath = configuration.getGeneratedDslName().toLowerCase() + NATIVES_PACKAGE;
		}
	}
	...
	public void invoke(@NotNull Project project, Editor editor, PsiFile file) throws IncorrectOperationException {
		...
		PsiClass aClass;
		if (variable.type().equals(Primitive.NATIVE))
			aClass = createNativeClass(variable, findDestiny(file, srcPsiDirectory, nativesPath));
		else aClass = createRuleClass(file, srcPsiDirectory);
	...
	private PsiClass createNativeClass(Variable variable, PsiDirectory destiny) {

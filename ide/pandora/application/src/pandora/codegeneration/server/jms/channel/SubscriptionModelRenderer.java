...
import pandora.codegeneration.action.SubscriptionActionRenderer;
...
public class SubscriptionModelRenderer {
	...
	private void processModel(SubscriptionModel subscription) {
		...
		Commons.writeFrame(new File(gen, "subscriptions"), snakeCaseToCamelCase(subscription.name()) + "Subscription", template().format(frame));

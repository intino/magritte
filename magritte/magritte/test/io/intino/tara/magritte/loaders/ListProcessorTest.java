...
public class ListProcessorTest {
	...
	public void should_execute_method_without_strings() throws Exception {
		List<?> list = ListProcessor.process(asList(1, "$@CodedInteger"), new MockLayer(new Node("mockito")));
	...
	public void should_execute_method_in_native_using_strings() throws Exception {
		List<?> list = ListProcessor.process(asList("Hey hey none", "$@CodedString"), new MockLayer(new Node("mockito")));
		assertThat(list.size(), is(2));

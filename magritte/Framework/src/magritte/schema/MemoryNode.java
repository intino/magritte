...
public class MemoryNode implements Node {
    ...
    private final GroupSet<Link, Node> links;
    ...
    @Override
    public Node type() {
        return links.group(Class).get(0);
    }

    @Override
    public Set<Node> types() {
        return links.group(Class);
    }

    @Override
    public Node parent() {
        return links.group(Parent).get(0);
    }

    @Override
    public Set<Node> children() {
        return links.group(Child);
    }
    ...
    @Override
    public Node owner() {
        return links.group(Owner).get(0);
    }

    @Override
    public Set<Node> members(Member member) {
        Link[] linkOf = {OwnedMember, RequiredMember, OptionalMember, AggregableMember };
        return links.group(linkOf[member.index]);
    }

    @Override
    public Set<Node> fanOut() {
        return links.group(FanOut);
    }

    @Override
    public Set<Node> fanIn() {
        return this.links.group(FanIn);
    }
    ...
    private String label() {
        return is(Case) ? "Case" : (is(Prototype) ? "Prototype" : "Type");
    }

...
import static siani.tara.magritte.wraps.Operation.Remove;
...
public class Morph implements NodeWrap, Cloneable {
	...
	public boolean is(Class<? extends NodeWrap> morphClass) {
		return check(node, Extract.nameOf(morphClass));
	}
	...
	public <T extends Morph> T _owner(Class<T> morphClass) {
		...
		while (owner != null) {
			if (check(owner, Extract.nameOf(morphClass))) return cast(owner).as(morphClass);
			owner = owner.owner();
		}
		return null;
	}
	...
	private Node type() {
		for (Node type : node.types())
			if (check(type, Extract.nameOf(this.getClass()))) return type;
		return node.type();
	}

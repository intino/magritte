...
import siani.tara.magritte.schema.GroupSet;
...
public class AcceptedGroupSets {
	...
	public void should_return_empty_groups_when_created() throws Exception {
		...
		assertTrue(set.group(AstronomicalBody.Satellite).asList().isEmpty());
		assertTrue(set.group(AstronomicalBody.Star).asList().isEmpty());
		assertTrue(set.group(AstronomicalBody.Planet).asList().isEmpty());
	}
	...
	public void should_remove_items_in_a_group() throws Exception {
		...
		assertEquals(2, set.group(AstronomicalBody.Star).size());
		assertEquals(3, set.group(AstronomicalBody.Planet).size());
		assertFalse(set.group(AstronomicalBody.Satellite).asList().isEmpty());
		...
		set.group(AstronomicalBody.Star).asList().remove("rigel");
		assertEquals(0, set.group(AstronomicalBody.Satellite).size());
		assertTrue(set.group(AstronomicalBody.Satellite).asList().isEmpty());
		assertEquals(3, set.group(AstronomicalBody.Planet).size());
		assertEquals("earth", set.group(AstronomicalBody.Planet).get(0));
	...
	public void should_index_items_in_each_group() throws Exception {
		...
		assertEquals(0, set.group(AstronomicalBody.Planet).asList().indexOf("europe"));
		assertEquals(1, set.group(AstronomicalBody.Satellite).asList().indexOf("europe"));
		assertEquals(-1, set.group(AstronomicalBody.Star).asList().indexOf("europe"));
	}

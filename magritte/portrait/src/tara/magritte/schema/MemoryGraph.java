...
import tara.magritte.Set.Filter;
...
public class MemoryGraph implements Graph {
    ...
    private List<Node> find(Set<Node> nodes, Filter<Node> filter) {
        ...
        nodes.forEach(node -> result.addAll(find(node.members(Node.Member.Component), filter)));
        return result;
    }
    ...
    public void index(Node node) {
        ...
        if (node.is(Tag.Case) && node.is(Tag.Root)) indexAsRoot(node);
    }
    ...
    private void indexAsRoot(Node node) {
        if (roots.contains(node) || !node.is(Tag.Case)) return;
        roots.add(node);
    }
    ...
    private List<Node> typesFromNodes() {
        ...
        for (String name : nodes.keySet()) {
            ...
            if (node.is(Tag.Case) || result.contains(node)) continue;
            result.add(node);
        }
        return result;
    }

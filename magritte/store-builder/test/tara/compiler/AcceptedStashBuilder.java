...
public class AcceptedStashBuilder {
	...
	public void should_build_empty_stash() {
		...
		assertThat(stashFrom(new File(home, "Empty.stash")).cases.length, is(0));
	}
	...
	public void should_build_stash_with_roots() {
		...
		assertThat(stashFrom(new File(home, "Months.stash")).cases.length, is(12));
	}
	...
	public void should_build_stash_with_entry_and_blobs() {
		...
		assertThat(stash.cases.length, is(2));
		assertThat("Asia has 1 component", stash.cases[0].cases.length, is(1));
		assertThat("Asia has City component", stash.cases[0].cases[0].types[0], is("City"));
		assertThat("Asia has City component named Tokyo", stash.cases[0].cases[0].name, is("Tokyo"));
		assertThat("Blob variable has right value", stash.cases[0].cases[0].variables[1].v, is("%World$1"));
	}
	...
	public void should_transform_instant_to_date() {
		...
		assertThat(stash.cases.length, is(1));
		assertThat(stash.cases[0].variables.length, is(4));
		assertThat(stash.cases[0].variables[0].v, instanceOf(long.class));
	}
	...
	public void should_build_stash_with_facets() {
		...
		assertThat(stashFrom(new File(home, "Temperature.stash")).cases.length, is(9));
	}
	...
	public void should_build_stash_with_many_entries() {
		...
		assertThat(stash.cases.length, is(1380));
		assertThat(stash.cases[10].variables.length, is(7));
		assertThat(stash.cases[10].variables[0].n, is("instant"));
		assertThat(stash.cases[10].variables[0].v, is(1320040860000L));
		assertThat(stash.cases[10].variables[5].v, is(23.0));
		assertThat(stash.cases[1000].variables[5].v, is(32.9));
	}
	...
	public void should_build_stash_with_passes() {
		...
		assertThat(stash.cases.length, is(24));
		for (Case component : stash.cases)
			assertThat("Root is Temperature", Arrays.asList(component.types).contains("Temperature"));
		assertThat("Temperature has city variable", stash.cases[0].variables[0].n, is("city"));
		assertThat("Temperature has month variable", stash.cases[0].variables[1].n, is("month"));
		assertThat("Temperature has temperature variable", stash.cases[0].variables[2].n, is("temperature"));
		assertThat("temperature variable has right value", stash.cases[0].variables[2].v, is(7.0));
		assertThat("Temperature Root Node has not cases", stash.cases[1].cases, is(nullValue()));
		assertThat("city variable of 1º element has correct reference", stash.cases[0].variables[0].v, is("!World.tara#Asia.Tokyo"));
		assertThat("city variable of 15º element has correct reference", stash.cases[15].variables[0].v, is("!World.tara#Europe.London"));
	}

...
import static io.intino.tara.Query.SortCriteria.Ascending;

public class Query {
    private Index index;
    private List<Index.Triple> selection;
    ...
    private Set<String> subjects(Predicate<Index.Triple> predicate) {
        return triplesWith(predicate).map(t -> t.subject).collect(toSet());
    }

    private Stream<Index.Triple> triplesWith(Predicate<Index.Triple> predicate) {
        return selection.stream().filter(predicate);
    }

    private Predicate<Index.Triple> with(String value) {
        return t -> t.contains(value);
    }

    private Predicate<Index.Triple> with(int id) {
        return t -> id >= 0 && t.contains(id);
    }

    private Predicate<Index.Triple> withAttribute(String attribute) {
        return t -> t.attribute.equals(attribute);
    }

    private Predicate<Index.Triple> withAttribute(String attribute, String value) {
    ...
    public class QueryGroup {
        ...
        public QueryGroup(Stream<Index.Triple> triples) {
        ...
        private String labelOf(Index.Triple triple) {
            return triple.text();
        }
    }
    ...
    private static class Sorting {
        ...
        public Sorting with(Stream<Index.Triple> triples) {

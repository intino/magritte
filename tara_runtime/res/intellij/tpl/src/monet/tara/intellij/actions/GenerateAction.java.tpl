...
public class GenerateAction extends AnAction implements DumbAware {
	...
	public void actionPerformed(final AnActionEvent e) {
		...
		final ::projectProperName::FileImpl[] ::projectName::Files = filterFiles(ModuleRootManager.getInstance(module).getSourceRoots(), project);
		createGenPath(project,::projectName::Files[0]);
			...
			public void run(\@NotNull ProgressIndicator indicator) {
				...
				ApplicationManager.getApplication().runReadAction(new Runnable() {
					...
					public void run() {
						...
						try {
//							new PluginGenerator(::projectName::Files, outputPath).generate();
							Notifications.Bus.notify(new Notification("Plugin Generator",
								::projectName::Files[0].getProject().getName() + " plugin generated", "to " + outputPath,
								NotificationType.INFORMATION), project);
						} catch (Exception ex) {
							Notifications.Bus.notify(new Notification("Plugin Generator",
								::projectName::Files[0].getProject().getName() + " plugin generation failed",null,null),project);
...
//								NotificationType.ERROR), project);
	...
	private ::projectProperName::FileImpl[] filterFiles(VirtualFile[] sourceRoots, Project project) {
		...
		return filteredFiles.toArray(new ::projectProperName::FileImpl[filteredFiles.size()]);
	}
	...
	private static void createGenPath(Project project, ::projectProperName::FileImpl ::projectName::File){
		final Module module = ProjectRootManager.getInstance(project).getFileIndex().getModuleForFile(::projectName::File.getVirtualFile());
		assert module != null;

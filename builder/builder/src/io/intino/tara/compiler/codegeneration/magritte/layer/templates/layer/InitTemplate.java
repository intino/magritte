...
public class InitTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "variable & owner & multiple")), (condition("type", "time | date")), not(condition("type", "concept | inherited | overriden")), (condition("trigger", "init"))).add(literal("if (name.equalsIgnoreCase(\"")).add(mark("name", "firstLowercase")).add(literal("\")) this.")).add(mark("name", "javaValidName", "firstLowerCase", "javaValidWord")).add(literal(" = io.intino.tara.magritte.loaders.")).add(mark("type", "lowercase", "capitalize")).add(literal("Loader.load(values, this);")),
			rule().add((condition("type", "variable & owner")), (condition("type", "time | date")), not(condition("type", "concept | multiple | inherited | overriden")), (condition("trigger", "init"))).add(literal("if (name.equalsIgnoreCase(\"")).add(mark("name", "firstLowercase")).add(literal("\")) this.")).add(mark("name", "javaValidName", "firstLowerCase", "javaValidWord")).add(literal(" = io.intino.tara.magritte.loaders.")).add(mark("type", "lowercase", "capitalize")).add(literal("Loader.load(values, this).get(0);")),
			rule().add((condition("type", "variable & resource & owner")), not(condition("type", "multiple | concept | inherited | overriden")), (condition("trigger", "init"))).add(literal("if (name.equalsIgnoreCase(\"")).add(mark("name", "firstLowercase")).add(literal("\")) this.")).add(mark("name", "javaValidName", "firstLowerCase", "javaValidWord")).add(literal(" = io.intino.tara.magritte.loaders.ResourceLoader.load(values, this).get(0);")),

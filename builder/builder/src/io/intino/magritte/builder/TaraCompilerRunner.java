...
import io.intino.magritte.builder.compiler.TaraCompiler;
import io.intino.magritte.builder.compiler.core.*;
import io.intino.magritte.builder.compiler.core.operation.generation.GenerateLanguageOperation;
import io.intino.magritte.builder.compiler.core.operation.generation.LayerGenerationOperation;
import io.intino.magritte.builder.compiler.core.operation.generation.StashGenerationOperation;
...
//		if (!sources.containsValue(false)) CompilationUnit.cleanOut(config);
	...
	private List<TaraCompiler.OutputItem> compileSources(CompilerConfiguration config, Map<File, Boolean> sources, List<CompilerMessage> messages) {
		...
		unit.addPhaseOperation(new LayerGenerationOperation(unit), Phases.CODE_GENERATION);
		unit.addPhaseOperation(new StashGenerationOperation(unit), Phases.CODE_GENERATION);
		if (!config.isTest()) unit.addPhaseOperation(new GenerateLanguageOperation(unit), Phases.CODE_GENERATION);

...
public class NativesCreator {
	...
	private Map<File, String> createNativeParameterClasses(List<Parameter> parameters, Map<String, String> originToDestiny) {
		...
		parameters.forEach(p -> {
			...
			builder.register(Parameter.class, new NativeParameterAdapter(conf.language(), outDSL, conf.level(), conf.workingPackage(), conf.dslWorkingPackage(), calculatePackage(p.container()), conf.getImportsFile()));
	...
	private Map<File, String> createNativeVariableClasses(List<Variable> natives, Map<String, String> files) {
		...
		natives.forEach(variable -> {
			FrameBuilder builder = new FrameBuilder();
			builder.register(Variable.class, new NativeVariableAdapter(conf.language(), outDSL, conf.workingPackage(), conf.dslWorkingPackage(), calculatePackage(variable.container()), conf.getImportsFile()));

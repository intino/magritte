...
public class ToNativeTransformerTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "native")), (condition("type", "date"))).add(literal("tara.magritte.loaders.DateLoader.load(java.util.Collections.singletonList(\"")).add(mark("value")).add(literal("\"), self).get(0)")),
			rule().add((condition("type", "native")), (condition("type", "instant"))).add(literal("tara.magritte.loaders.InstantLoader.load(java.util.Collections.singletonList(\"")).add(mark("value")).add(literal("\"), self).get(0)")),

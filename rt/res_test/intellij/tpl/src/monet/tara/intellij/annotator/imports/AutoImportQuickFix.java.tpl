...
public class AutoImportQuickFix implements LocalQuickFix, HighPriorityAction {
	...
	private final Definition myNode;
	 ...
	 */
	public AutoImportQuickFix(Definition node, PsiReference reference, boolean qualify) {
		myNode = node;
	 ...
	 */
	public void addImport(\@NotNull PsiElement importable, \@NotNull PsiFile file, \@Nullable ::projectProperName::PsiElement importElement) {
		myImports.add(new ImportCandidateHolder(importable, file, (::projectProperName::ImportStatement) importElement, null));
	}
	...
	public String getText() {
		if (myUseQualifiedImport) return "MODULE";//::projectProperName::Bundle.message("ACT.qualify.with.module");
		else if (myImports.size() == 1) {
			return "Import '" + myImports.get(0).getPresentableText(getNameToImport()) + "'";
		} else {
			return "USE IMPORT";//::projectProperName::Bundle.message("ACT.NAME.use.import");
		}
	}
	...
	public String getFamilyName() {
		return "Import";//::projectProperName::Bundle.message("ACT.FAMILY.import");
	}

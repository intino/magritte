...
public class ParameterAnnotator extends ::projectProperName::Annotator {
	...
	public void annotate(\@NotNull PsiElement element, \@NotNull AnnotationHolder holder) {
		...
		ASTNode node = ::projectProperName::Language.getHeritage().getNodeNameLookUpTable().get(metaIdentifier.getText()).get(0);
	...
	private void processAsWordOrReference(PsiElement element, AnnotationHolder holder, ASTNode.Variable actualVariable) {
		if (actualVariable instanceof ASTNode.Word)
			processAsWord((::projectProperName::IdentifierReference) element.getFirstChild(), ((ASTNode.Word) actualVariable));
		else if (element instanceof ::projectProperName::IdentifierReference &&
			checkReference((::projectProperName::IdentifierReference) element.getFirstChild(), (ASTNode.Reference) actualVariable))
			holder.createErrorAnnotation(element, "Parameter type error");
	}

	private boolean processAsWord(::projectProperName::IdentifierReference reference, ASTNode.Word word) {
		return word.contains(getLastElementOf(reference).getText());
	}

	private boolean checkReference(::projectProperName::IdentifierReference reference, ASTNode.Reference variable) {

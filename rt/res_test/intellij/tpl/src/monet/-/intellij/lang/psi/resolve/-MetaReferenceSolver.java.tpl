...
public class ::projectProperName::MetaReferenceSolver extends PsiReferenceBase<PsiElement> implements PsiPolyVariantReference {
	...
	public Object[] getVariants() {
		...
		if (myElement instanceof MetaIdentifier) {
			...
			if (context != null) {
				AbstractNode node = ::projectProperName::Language.getHeritage().getNodeNameLookUpTable().get(context.getType()).get(0);
				if (node != null) {
	...
	private void addBaseDefinitions(String baseDefinition, List<String> definitions) {
		AbstractNode node = ::projectProperName::Language.getHeritage().getNodeNameLookUpTable().get(baseDefinition).get(0);
	...
	private void addInheritedDefinitions(String extendFrom, List<String> definitions) {
		AbstractNode node = ::projectProperName::Language.getHeritage().getNodeNameLookUpTable().get(extendFrom).get(0);
		for (AbstractNode children \: node.getChildren())
			if (!children.isBase() && !children.isAbstract()) definitions.add(children.getIdentifier());
		if (node.getParentName() != null) addInheritedDefinitions(node.getParentName(), definitions);
	}

	private void addChildren(List<String> definitions, AbstractNode node) {
		for (AbstractNode child \: node.getChildren())
			if (!child.isBase() && !child.isAbstract())
				...
				if (abstractNode.isCase())
					definitions.add(abstractNode.getIdentifier());
	}

...
public class ::projectProperName::MoveDefinitionProcessor extends BaseRefactoringProcessor {
	public static final String REFACTORING_NAME = ::projectProperName::Bundle.message("refactoring.move.definition");

	private PsiNamedElement[] myElements;
	private String myDestination;

	public ::projectProperName::MoveDefinitionProcessor(Project project, PsiNamedElement[] elements, String destination, boolean previewUsages) {
		super(project);
	...
	private static void moveElement(\@NotNull PsiNamedElement element, \@NotNull Collection<UsageInfo> usages, \@NotNull ::projectProperName::File destination) {
		final PsiFile file = element.getContainingFile();
//		::projectProperName::DefinitionRefactoringUtil.rememberNamedReferences(element);
...
//		::projectProperName::DefinitionRefactoringUtil.restoreNamedReferences(newElement, element);
		// TODO\: Remove extra empty lines after the removed element
		element.delete();
		if (file != null) optimizeImports(file);
	}

	private static PsiNamedElement addToFile(\@NotNull PsiNamedElement element, \@NotNull final ::projectProperName::File destination, \@NotNull Collection<UsageInfo> usages) {
...
//		if (usage instanceof ::projectProperName::QualifiedExpression) {
//			::projectProperName::QualifiedExpression expr = (::projectProperName::QualifiedExpression) usage;
//			if (oldElement instanceof ::projectProperName::Class && ::projectProperName::Names.INIT.equals(expr.getName())) {
//				return;
//			}
//			if (expr.isQualified()) {
//				final ::projectProperName::ElementGenerator generator = ::projectProperName::ElementGenerator.getInstance(expr.getProject());
//				final ::projectProperName::Expression generated = generator.createExpressionFromText(LanguageLevel.forElement(expr), expr.getName());
//				final PsiElement newExpr = expr.replace(generated);
//				::projectProperName::ClassRefactoringUtil.insertImport(newExpr, newElement, null, true);
//			}
//		}
//		if (usage instanceof ::projectProperName::StringLiteralExpression) {
...
//			}
//		} else {
//			final ::projectProperName::ImportStatementBase importStmt = getImportStatementByElement(usage);
//			if (importStmt != null) {
//				::projectProperName::ClassRefactoringUtil.updateImportOfElement(importStmt, newElement);
//			}
...
//				::projectProperName::ClassRefactoringUtil.insertImport(oldElement, newElement);
//			}
//			if (resolvesToLocalStarImport(usage)) {
//				::projectProperName::ClassRefactoringUtil.insertImport(usage, newElement);
//				if (usageFile != null) {
...
//		new ::projectProperName::ImportOptimizer().processFile(file).run();
	}
...
//				if (resolved instanceof ::projectProperName::StarImportElement && resolved.getContainingFile() == containingFile) {
//					return true;
//				}
//			}
		}
...
//		if (!::projectProperName::DefinitionRefactoringUtil.isValidQualifiedName(qName)) {
//			throw new IncorrectOperationException(::projectProperName::Bundle.message("refactoring.move.definition.error.cannot.use.module.name.$0", qName));
//		}
	}
	...
	protected UsageInfo[] findUsages() {
		...
		for (PsiNamedElement element \: myElements) {
			usages.addAll(::projectProperName::RefactoringUtil.findUsages(element, false));
		}
		...
		CommandProcessor.getInstance().executeCommand(myElements[0].getProject(), new Runnable() {
			public void run() {
				ApplicationManager.getApplication().runWriteAction(new Runnable() {
					public void run() {
						final ::projectProperName::File destination = ::projectProperName::Util.getOrCreateFile(myDestination, myProject);
						...
						for (PsiNamedElement e \: myElements) {
							...
							if (e instanceof Definition && destination.getDefinition() != null) {
								throw new IncorrectOperationException(::projectProperName::Bundle.message("refactoring.move.definition.error.destination.file.contains.definition.$0",
									e.getName()));
							}

...
public class ModelTemplate extends Template {
	...
	public Template define() {
		add(
			...
			rule().add((condition("type", "root")), not(condition("type", "single")), (condition("trigger", "root"))).add(literal("public magritte.Set<")).add(mark("qn", "reference")).add(literal("> ")).add(mark("name", "javaValidName", "plural")).add(literal("() {\n    return _get(\"")).add(mark("name", "FirstLowerCase", "plural")).add(literal("\", ")).add(mark("qn", "reference")).add(literal(".class);\n}\n\npublic ")).add(mark("qn", "reference")).add(literal(" ")).add(mark("name", "javaValidName")).add(literal("(int index) {\n    return ")).add(mark("name", "javaValidName", "plural")).add(literal("().get(index);\n}")),
			rule().add((condition("type", "root")), (condition("type", "single")), (condition("trigger", "root"))).add(literal("public ")).add(mark("qn", "reference")).add(literal(" ")).add(mark("name", "javaValidName")).add(literal("() {\n    return _get(\"")).add(mark("name", "FirstLowerCase")).add(literal("\", ")).add(mark("qn", "reference")).add(literal(".class).get(0);\n}"))
		);
		return this;
	}
}
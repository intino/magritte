...
public class ModelTemplate extends Template {
	...
	public Template define() {
		add(
			rule().add((condition("type", "scene"))).add(literal("package ")).add(mark("name", "LowerCase")).add(literal(";\n\npublic class ")).add(mark("name", "FirstCamelCase")).add(literal("Model extends magritte.wraps.Model {\n\n    public ")).add(mark("name", "FirstCamelCase")).add(literal("Model(magritte.wraps.Morph morph) {\n        super(morph);\n    }\n\n    public ")).add(mark("name", "FirstCamelCase")).add(literal("Model(magritte.Graph graph) {\n\t\tsuper(graph);\n\t}\n\n\t")).add(mark("root").multiple("\n\n")).add(literal("\n}")),

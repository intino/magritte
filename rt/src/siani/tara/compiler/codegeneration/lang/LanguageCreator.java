...
//				for (NodeImpl node : nodeTree)
//					buildNode(node);
//			}
//
//			private void buildNode(NodeImpl node) {
...
//					for (NodeImpl inner : filter(node.getInnerNodes())) {
//						if (inner.is(LinkNode.class) && ((LinkNode) inner).isReference()) continue;
//						buildNode(inner);
//					}
//				}
//				for (NodeImpl inner : node.getSubNodes())
//					buildNode(inner);
//			}
//
...
//			}
//
//			private void addAllows(NodeImpl node, Frame frame) {
...
//			}
//
//			private void addRequires(NodeImpl node, Frame frame) {
...
//			}
//
//			private void addContextAllows(NodeImpl node, Frame allows) {
...
//			}
//
//			private void addFacetAllows(NodeImpl node, Frame allows) {
...
//			}
//
//			private void addContextRequires(NodeImpl node, Frame requires) {
//				for (int i = 0; i < node.getObject().getVariables().size(); i++) {
//					Variable variable = node.getObject().getVariables().get(i);
...
//				NodeImpl node = model.searchNode(reference.getType());
...
//				for (NodeImpl node : collectCandidates(tree)) {
...
//			}
//
//			private Collection<NodeImpl> collectCandidates(NodeTree tree) {
//				List<NodeImpl> nodes = new ArrayList<>();
//				for (NodeImpl node : tree)

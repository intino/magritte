...
public abstract class TaraCompilerBase implements TranslatingCompiler {
	...
	protected void runTaracCompiler(final CompileContext compileContext, final Module module,
...
//		}
//
//		final List<String> patchers = new SmartList<>();
//
...
//		}
//
//		final TaraCompilerConfiguration compilerConfiguration = TaraCompilerConfiguration.getInstance(myProject);
...
//		}
	...
	protected Set<VirtualFile> enumerateGroovyFiles(final Module module) {
		final Set<VirtualFile> moduleClasses = new THashSet<>();
		ModuleRootManager.getInstance(module).getFileIndex().iterateContent(new ContentIterator() {
			public boolean processFile(final VirtualFile vfile) {
				if (!vfile.isDirectory() &&
					...
					try {
						if (PsiManager.getInstance(myProject).findFile(vfile) instanceof TaraFile) {
	...
	protected static void addStubsToCompileScope(List<String> outputPaths, CompileContext compileContext, Module module) {
		List<VirtualFile> stubFiles = new ArrayList<>();
	...
	private static ModuleChunk createChunk(Module module, CompileContext context) {
		return new ModuleChunk((CompileContextEx) context, new Chunk<>(module), Collections.<Module, List<VirtualFile>>emptyMap());
	}
	...
	public void compile(final CompileContext compileContext, Chunk<Module> moduleChunk, final VirtualFile[] virtualFiles, OutputSink sink) {
		...
		for (final Module module : moduleChunk.getNodes()) {
			...
			final List<VirtualFile> toCompile = new ArrayList<>();
			final List<VirtualFile> toCompileTests = new ArrayList<>();
...
//			}
	...
	public boolean isCompilableFile(VirtualFile file, CompileContext context) {
		final boolean result = TaraFileType.INSTANCE.equals(file.getFileType());

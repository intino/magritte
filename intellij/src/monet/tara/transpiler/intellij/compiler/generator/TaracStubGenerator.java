package monet.tara.transpiler.intellij.compiler.generator;
...
import monet.tara.transpiler.intellij.compiler.TaraCompilerBase;
import monet.tara.transpiler.intellij.compiler.TaraCompilerConfiguration;
...
public class TaracStubGenerator extends TaraCompilerBase {
	...
	private static Logger LOG = Logger.getInstance("monet.tara.transpiler.intellij.compiler.generator.TaracStubGenerator");

	public static final String TARA_STUBS = "taraStubs";

	public TaracStubGenerator(Project project) {
		super(project);
	}
...
//		}
	...
	private static File getStubOutput(Module module, boolean tests) {
		...
		final String rootPath = CompilerPaths.getGeneratedDataDirectory(project).getPath() + "/" + TARA_STUBS + "/";
	...
	public static PsiClass findClassByStub(Project project, VirtualFile stubFile) {
		...
		final int stubs = Arrays.asList(components).indexOf(TARA_STUBS);
...
//
//		try {
//			output = generator.generateStubs((TaraFile) PsiManager.getInstance(project).findFile(item));
//		}
//		finally {
//			accessToken.finish();
//		}

...
public class ConceptTreeView implements Navigatable {
	public static final DataKey<ConceptTreeView[]> DATA_KEY = DataKey.create("form.array");

	private final Collection<PsiFile> taraFiles;
	private final PsiClass myClassToBind;

	public ConceptTreeView(PsiClass classToBind, Collection<PsiFile> formFiles) {
		myClassToBind = classToBind;
		taraFiles = new HashSet<>(formFiles);
	}

	public boolean equals(Object object) {
		if (object instanceof ConceptTreeView) {
			ConceptTreeView form = (ConceptTreeView) object;

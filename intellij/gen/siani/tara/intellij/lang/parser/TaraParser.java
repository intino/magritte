...
public class TaraParser implements PsiParser {
  ...
  /* ********************************************************** */
  // (NEW_LINE_INDENT | INLINE) (nodeConstituents NEWLINE+)+ DEDENT
  ...
  public static boolean measureType(PsiBuilder b, int l) {
    ...
    if (!nextTokenIs(b, "<measure type>", IDENTIFIER_KEY, MEASURE_VALUE_KEY)) return false;
    boolean r;
    Marker m = enter_section_(b, l, _NONE_, "<measure type>");
    r = consumeToken(b, MEASURE_VALUE_KEY);
    if (!r) r = consumeToken(b, IDENTIFIER_KEY);
    exit_section_(b, l, m, MEASURE_TYPE, r, false, null);
    return r;
  }
  ...
  /* ********************************************************** */
  // NEWLINE* dslDeclaration? imports? (node NEWLINE+)*
  ...
  /* ********************************************************** */
  // (subNode | metaIdentifier parameters? identifier? parent?) annotations? address?
  ...
  // integerValue+
  private static boolean value_5_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "value_5_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    ...
    while (r) {
      ...
      if (!empty_element_parsed_guard_(b, "value_5_0", c)) break;
      c = current_position_(b);
    }
    exit_section_(b, m, null, r);
    return r;
  ...
  // measureValue?
  private static boolean value_5_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "value_5_1")) return false;
    measureValue(b, l + 1);
    return true;
  }

  // doubleValue+  measureValue?
  private static boolean value_6(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "value_6")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = value_6_0(b, l + 1);
    r = r && value_6_1(b, l + 1);
  ...
  // doubleValue+
  private static boolean value_6_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "value_6_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = doubleValue(b, l + 1);
    int c = current_position_(b);
    while (r) {
      ...
      if (!empty_element_parsed_guard_(b, "value_6_0", c)) break;
      c = current_position_(b);
    }
    exit_section_(b, m, null, r);
    return r;
  ...
  // measureValue?
  private static boolean value_6_1(PsiBuilder b, int l) {
  ...
  /* ********************************************************** */
  // VAR variableType attributeType? (LIST | count)? IDENTIFIER_KEY (word | (EQUALS value measureValue?))? annotations?
  ...
  private static boolean word_1(PsiBuilder b, int l) {
    ...
    int c = current_position_(b);
    while (r) {
      if (!word_1_0(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "word_1", c)) break;
      c = current_position_(b);
    }
    exit_section_(b, m, null, r);
    return r;
  ...
  // STAR?
  private static boolean word_1_0_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "word_1_0_1")) return false;
    consumeToken(b, STAR);
    return true;
  }

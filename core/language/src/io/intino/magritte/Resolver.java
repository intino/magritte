...
public class Resolver {
	...
	private List<Constraint> contextConstraints(Node node) {
		...
		if (constraints != null && (isComponent(constraints, node) || isMetaAspect(constraints, node)))
			return constraints;
	...
	private List<Constraint.MetaAspect> metaAspects(List<Constraint> context) {
		return context.stream().filter(c -> c instanceof Constraint.MetaAspect).map(c -> (Constraint.MetaAspect) c).collect(toList());
	}
	...
	private void checkMetaAspectConstraint(Node node, Constraint.MetaAspect constraint) {
		if (node.type() != null && shortType(node.type()).equals(shortType(constraint.type())))
	...
	private void resolveAspects(Node node) {
		for (Aspect aspect : node.appliedAspects()) {
			Constraint.Aspect constraint = (Constraint.Aspect) language.constraints(node.type()).stream().
					filter(c -> c instanceof Constraint.Aspect && simpleType((Constraint.Aspect) c).equals(aspect.type())).findFirst().orElse(null);
			if (constraint != null) {
				...
				resolveParameters(aspect.parameters(), parameterConstraints);
			}
		}
	}

	private void resolveParameters(Node node) {
		List<Constraint.Parameter> constraints = language.constraints(node.type()).stream()
				.filter(c -> c instanceof Constraint.Parameter)
				.map(c -> (Constraint.Parameter) c)
				.collect(toList());
		resolveParameters(node.parameters(), constraints);
	}
	...
	private static void resolveParameters(List<Parameter> parameters, List<Constraint.Parameter> constraints) {
		for (Parameter param : parameters) {
			...
			if (constraint != null && param.name().isEmpty()) param.name(constraint.name());
		}
	}
